...
{
	EncodingCvt ecv;
	encoding_cvt_func_t tp_convert_encoding = NULL;
	...
	if(torrent->src_encoding == NULL){
		if(stricmp(torrent->dst_encoding,UTF8_ENCODING)==0){
			tp_convert_encoding = encoding_cvt_to_utf8;
		}
		else if(stricmp(torrent->dst_encoding,GBK_ENCODING)==0){
			tp_convert_encoding = encoding_cvt_to_gbk;
		}
		else if(stricmp(torrent->dst_encoding,BIG5_ENCODING)==0){
			tp_convert_encoding = encoding_cvt_to_big5;
		}
	}
	else if(stricmp(torrent->src_encoding,UTF8_ENCODING)==0){
		if(stricmp(torrent->dst_encoding,GBK_ENCODING)==0){
			tp_convert_encoding = encoding_cvt_utf8_to_gbk;
		}
		else if(stricmp(torrent->dst_encoding,BIG5_ENCODING)==0){
			tp_convert_encoding = encoding_cvt_utf8_to_big5;
		}
	}
	else if(stricmp(torrent->src_encoding,GBK_ENCODING)==0){
		if(stricmp(torrent->dst_encoding,UTF8_ENCODING)==0){
			tp_convert_encoding = encoding_cvt_gbk_to_utf8;
		}
		else if(stricmp(torrent->dst_encoding,BIG5_ENCODING)==0){
			tp_convert_encoding = encoding_cvt_gbk_to_big5;
		}
	}
	else if(stricmp(torrent->src_encoding,BIG5_ENCODING)==0){
		if(stricmp(torrent->dst_encoding,UTF8_ENCODING)==0){
			tp_convert_encoding = encoding_cvt_big5_to_utf8;
		}
		else if(stricmp(torrent->dst_encoding,GBK_ENCODING)==0){
			tp_convert_encoding = encoding_cvt_big5_to_gbk;
		}
	}
	if(tp_convert_encoding == NULL) return 0;
	
	encoding_cvt_initialize(&ecv);

	dst_buff =(char *)alloca(dst_len);
	//convert title name to destination encoding
	result = tp_convert_encoding(&ecv, torrent->_title_name,torrent->_title_name_len,dst_buff,&dst_len);
	if(result == 0){
	...
	for(p_sub_file = torrent->_file_list;p_sub_file != NULL;p_sub_file = p_sub_file->_p_next){
		if(p_sub_file->_file_path_len>0){
			dst_len = MAX_PATH;
			result = tp_convert_encoding(&ecv, p_sub_file->_file_path,p_sub_file->_file_path_len,dst_buff,&dst_len);
			if(result == 0){
		...
		dst_len = MAX_PATH;
		result = tp_convert_encoding(&ecv, p_sub_file->_file_name,p_sub_file->_file_name_len,dst_buff,&dst_len);
		if(result == 0){
	...
	for (parse_cur = 0; parse_cur < len; ++parse_cur) {
		...
		else if (p_item->_type == BENCODING_ITEM_INTEGER || p_item->_type == BENCODING_ITEM_NEGINT) 
		{
			...
			else if (input_char == 'e') 
			{
				...
				parsing_result = ERR_INVALID_PARAMETER;
				break;
			}
		} 
		else if (p_item->_type == BENCODING_ITEM_STRING) 
