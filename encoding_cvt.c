...
static void encoding_cvt_reset(iconv_t cd);
static int32_t encoding_cvt_conv(iconv_t cd, ecv_ctx_t* ctx, const char* in, uint32_t in_len, char* out, uint32_t* out_len);
...
{
	if(self->gbk_to_utf8 != NULL)
	{
		iconv_close(self->gbk_to_utf8);
		self->gbk_to_utf8 = NULL;
	}

	if(self->big5_to_utf8 != NULL)
	{
		iconv_close(self->big5_to_utf8);
		self->big5_to_utf8 = NULL;
	}

	if(self->big5_to_gbk != NULL)
	{
		iconv_close(self->big5_to_gbk);
		self->big5_to_gbk = NULL;
	}

	if(self->utf8_to_gbk != NULL)
	{
		iconv_close(self->utf8_to_gbk);
		self->utf8_to_gbk = NULL;
	}

	if(self->gbk_to_big5 != NULL)
	{
		iconv_close(self->gbk_to_big5);
		self->gbk_to_big5 = NULL;
	}

	if(self->utf8_to_big5 != NULL)
	{
...
int32_t encoding_cvt_gbk_to_utf8(EncodingCvt* self, const char* in, uint32_t in_len, char* out, uint32_t* out_len)
{
	...
	return encoding_cvt_conv(self->gbk_to_utf8, &gbk_to_utf8_ctx, in, in_len, out, out_len);
}
...
int32_t encoding_cvt_big5_to_utf8(EncodingCvt* self, const char* in, uint32_t in_len, char* out, uint32_t* out_len)
{
	...
	return encoding_cvt_conv(self->big5_to_utf8, &big5_to_utf8_ctx, in, in_len, out, out_len);
}
...
int32_t encoding_cvt_utf8_to_gbk(EncodingCvt* self, const char* in, uint32_t in_len, char* out, uint32_t* out_len)
{
	...
	return encoding_cvt_conv(self->utf8_to_gbk, &utf8_to_gbk_ctx, in, in_len, out, out_len);
}
...
int32_t encoding_cvt_big5_to_gbk(EncodingCvt* self, const char* in, uint32_t in_len, char* out, uint32_t* out_len)
{
	...
	return encoding_cvt_conv(self->big5_to_gbk, &big5_to_gbk_ctx, in, in_len, out, out_len);
}
...
int32_t encoding_cvt_gbk_to_big5(EncodingCvt* self, const char* in, uint32_t in_len, char* out, uint32_t* out_len)
{
	...
	return encoding_cvt_conv(self->gbk_to_big5, &gbk_to_big5_ctx, in, in_len, out, out_len);
}
...
int32_t encoding_cvt_utf8_to_big5(EncodingCvt* self, const char* in, uint32_t in_len, char* out, uint32_t* out_len)
{
	...
	return encoding_cvt_conv(self->utf8_to_big5, &utf8_to_big5_ctx, in, in_len, out, out_len);
}
...
int32_t encoding_cvt_to_utf8(EncodingCvt* self, const char* in, uint32_t in_len, char* out, uint32_t* out_len)
{
	...
	{ //gbk
		return encoding_cvt_gbk_to_utf8(self, in, in_len, out, out_len);
	}
	...
	{ //utf8
		return encoding_cvt_copy_input(in, in_len, out, out_len);
	}
	...
	{ //big5
		return encoding_cvt_big5_to_utf8(self, in, in_len, out, out_len);
	}
	...
	{ //gbk or utf8
		...
		{ //utf8
			if(invalid_utf8_num == 0) return encoding_cvt_copy_input(in, in_len, out, out_len);
...
int32_t encoding_cvt_to_gbk(EncodingCvt* self, const char* in, uint32_t in_len, char* out, uint32_t* out_len)
{
	...
	if (invalid_utf8_num == 0 && common_utf8_num > 4 * non_common_utf8_num)
	{ //utf8
		return encoding_cvt_utf8_to_gbk(self, in, in_len, out, out_len);
	}

	common_big5_num = get_str_big5_char_number(in, in_len, &non_common_big5_num, &invalid_big5_num);
	if (invalid_big5_num == 0 && common_big5_num > 4 * non_common_big5_num)
	{ //big5
		return encoding_cvt_big5_to_gbk(self, in, in_len, out, out_len);
	}
	...
	{ //gbk or utf8
		if (common_gbk_num >= common_utf8_num)
		{ //gbk
			if(invalid_gbk_num == 0) return encoding_cvt_copy_input(in, in_len, out, out_len);
...
int32_t encoding_cvt_to_big5(EncodingCvt* self, const char* in, uint32_t in_len, char* out, uint32_t* out_len)
{
	...
	if (invalid_gbk_num == 0 && common_gbk_num > 4 * non_common_gbk_num)
	{ //gbk
		return encoding_cvt_gbk_to_big5(self, in, in_len, out, out_len);
	}

	common_utf8_num = get_str_utf8_char_number(in, in_len, &non_common_utf8_num, &invalid_utf8_num);
	if (invalid_utf8_num == 0 && common_utf8_num > 4 * non_common_utf8_num)
	{ //utf8
		return encoding_cvt_utf8_to_big5(self, in, in_len, out, out_len);
	}
	...
	{ // big5 or utf8
		...
		{ //big5
			if(invalid_big5_num == 0) return encoding_cvt_copy_input(in, in_len, out, out_len);
...
int main()
{
	...
	uint32_t out_len;
	EncodingCvt ecv;
	int ret = encoding_cvt_initialize(&ecv);
	if(ret != 0)
	{
		perror("init conv ");
		return -1;
	...
	out_len = 16*1024;
	ret = encoding_cvt_to_gbk(&ecv, gbk_str, strlen(gbk_str), buf, &out_len);
	buf[out_len] = 0;
	printf("gbk :");
	print_hex(buf);

	out_len = 16*1024;
	ret = encoding_cvt_to_big5(&ecv, gbk_str, strlen(gbk_str), buf, &out_len);
	buf[out_len] = 0;
	printf("big5 :");
	print_hex(buf);

	out_len = 16*1024;
	ret = encoding_cvt_to_utf8(&ecv, gbk_str, strlen(gbk_str), buf, &out_len);
	buf[out_len] = 0;
	printf("utf8 :");
	print_hex(buf);
	...
	while(fgets(buf1, 16*1024, stdin))
	{
		...
		out_len = 16*1024;
		encoding_cvt_to_utf8(&ecv, buf1, n, buf, &out_len);
		buf[out_len] = 0;
		printf("output: ");
		print_hex(buf);
		puts(buf);
	...
	return encoding_cvt_finalize(&ecv);
}
